...
\begin{frame}{Regression with correct functional form}
  ...
  \begin{figure}
    \includegraphics[scale=0.17]{./lecture_includes/regression_fnform.png}
  \end{figure}
...
\begin{itemize}
  \item Reviewing the standard OLS model as referenced in Tymon Słoczyn's 2022 Restat paper.
  \item The equation to consider:
    \begin{equation}
      y = \alpha + \tau d + X \beta + u,
    \end{equation}
  \item This model is what you might call the standard OLS model with controls.
...
\begin{frame}
\frametitle{Bias from Constant Treatment Effects}
\begin{itemize}
  \item The standard OLS model assumes treatment effects are homogeneous across observations.
  \item But Tymon's paper addresses the interpretation of \(\tau\), the OLS estimand, when treatment effects are heterogeneous.
  \item He demonstrates that \(\tau\) is a convex combination of two parameters: ATT and ATU.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item The weight OLS places on the average effect for each group is inversely related to the group's size.
  \item As the number of treated units increases, the weight on ATT decreases.
  \item This suggests OLS estimates may be inappropriate when treatment effects vary across subjects.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Homogeneity assumptions are generally strong and may not hold when treatment effects vary significantly.
  \item Tymon's paper provides a pragmatic perspective on the main result and derives corollaries suggesting diagnostic methods for when:
    ...
    \end{enumerate}
    \item Remember you're an economist -- outside of the experiment, people typically \emph{choose} their treatments (i.e., crimes that yield some sentence)
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item LaLonde (1986) and Angrist and Pischke (2009) assessments using the Dehejia and Wahba (2002) subsample (see Stata code together)
    ...
    \end{itemize}
  \item Treated sample's size (1.1\%) implies ATE and ATU closeness.
\end{itemize}
\end{frame}
...
Theorem: Weighted Average Interpretation of OLS.  

\bigskip

Under some basic assumptions (see Tymon 2022) A1 and A2), then $$\tau = w_1 \times \tau_{APLE,1} + w_0 \times \tau_{APLE,0}$$
 
\begin{align*}
w_1 &= \frac{(1-\rho) \times V[p(X)|d=0]}{\rho \times V[p(X)|d=1] + (1-\rho) \times V[p(X)|d=0]} \\
\text{and} \quad w_0 &= 1 - \frac{\rho \times V[p(X)|d=1]}{\rho \times V[p(X)|d=1] + (1-\rho) \times V[p(X)|d=0]}
\end{align*}

\end{frame}

\begin{frame}
\frametitle{Interpretation of OLS weighted average theorem}

\begin{itemize}
\item The OLS estimand, $\widehat{\tau}$, is a convex combination of estimated average partial linear effects for the treatment group (,1) and control (,0)
\item The $\tau$ term is based on a three step procedure:
	\begin{enumerate}
	\item Calculate the propensity score, $p(X)$
	\item Calculate the linear project of the outcome on the propensity score separately for treatment and control (i.e., RA)
	\item Calculate a weighted average
	\end{enumerate}
\item The first weight, $w_1$, is decreasing $\frac{V[p(X )|d=1]}{V[p(X )|d=0]}$ and $\rho$ and the opposite for $w_0$ which means the more units in a group, the less weight it gets
\item Angrist and Pischke (2009) that OLS gives more weight to treatment effects that are better estimated in finite samples
\end{itemize}
\end{frame}
