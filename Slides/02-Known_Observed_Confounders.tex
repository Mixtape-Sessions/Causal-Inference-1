...
\begin{frame}{Measuring the inexact matching}

\begin{itemize}
\item Imagine I match treated units to some comparison unit -- questions is which one?
\item What if we had a way of measuring a match in terms of how ``close'' each unit's $X_i$ value was to the matched $X_j$
\item Well then we could see how ``bad'' it is
\item Let's do that and use the square root of the sum of all squared differences in each unit's $X_i - X_{j(i)}$
\end{itemize}
