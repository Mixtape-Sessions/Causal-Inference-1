...
\begin{frame}{Review the steps}

\begin{itemize}

\item First step is always to decide what population you're seeking to know the causal effect -- everyone (ATE), treated only (ATT), not treated (ATU); each has different assumptions
\item Next estimate the propensity score
\item Choose your estimation method: main ones are \emph{stratification}, \emph{weighting} or \emph{matching}
\item Final step will be calculating the standard error 
\item Let's review the steps now
...
\begin{itemize}
	\item With full unconfoundedness, you can estimate all three \emph{average} causal parameters, even the ATU
	\item But that requires strong assumptions that within the dimensions of $X$, summarized by the propensity score into a single scalar, all remaining variation in the treatment is ``as good as random''
	\item Something for you to think about -- which population are you really wanting to know the answer to?  Do you want to know the effect of discrimination on the discriminated group?  Then that's the ATT not the ATE
...
\begin{frame}{Step 2: Estimate the propensity score}
		...
		\item Estimate the conditional probability of treatment using probit or logit model (or ML) $$Pr(D_i=1|X_i) = F(\beta X_i)$$
			\begin{itemize}
			\item Don't use OLS because OLS overfits and places propensity scores outside of realism
...
\begin{itemize} 
\item Hahn (1998) studied IPW and calculated the semiparametric efficiency bounds, then proposed efficient estimators for the ATE and the ATT
\item His estimators were efficient regardless of whether the propensity score was known 
\item Nonparametrically estimated two conditional expectations, $E[YD|X=x]$ and $[Y(1-D)|X=x]$ as well as the propensity score, $\rho(X)$, and then imputed missing potential outcomes as $\widehat{Y_i^1=\frac{\widehat{E}[YD|X_i]}{\widehat{\rho}(X_i)}}$ and $\widehat{Y_i^0=\frac{\widehat{E}[Y(1-D)|X_i]}{\widehat{\rho}(X_i)}}$
\end{itemize}
...
\item     The two estimators are mathematically equivalent, but have different
	...
	\end{itemize}
\item The first estimator is usually referred to as the doubly-robust (DR) estimator
