...
\section{Thornton HIV Study}

\begin{frame}{What used to happen when people learned they were HIV Positive?}

  \begin{itemize}
    \item In the early 2000s, many believed HIV testing would help reduce risky behavior in Sub-Saharan Africa
    \item But few had tested whether people would actually pick up their results—especially when results might bring fear or stigma
  \end{itemize}
...
\begin{frame}{What used to happen when people learned they were HIV Positive?}
  ...
  \begin{itemize}
    \item \textbf{Rebecca Thornton}, then a student at Harvard's Economics and Government PhD program, had two questions which had never before been answered until her study: 
    	\begin{enumerate}
	\item Could small incentives increase demand for HIV test results?
	\item Would learning HIV test results change risky behaviors?
	\end{enumerate}
\item Very influential study in many respects for answering a question that had previously only been guessed at using "common sense"
  \end{itemize}
...
\begin{frame}{The Thornton Experiment (Malawi, 2004)}
  \begin{itemize}
    \item Conducted in rural Malawi as part of a panel survey on HIV/AIDS
    \item Participants offered a free at-home HIV test
    \item Then randomized to receive \textbf{a cash voucher of \$0 to \$3}
    \item Voucher could only be redeemed if they picked up their HIV results at a nearby VCT center
    \item Main outcomes: Did they pick up results? Did behavior change after?
  \end{itemize}
\end{frame}

\begin{frame}{Experimental Design}
  \begin{itemize}
  \item Thornton is \emph{not} randomizing HIV infection status
  \item Rather she is randomizing people's \textbf{knowledge} of their own HIV infection status by holding a random lottery
  	\begin{itemize}
	    \item Treatment variable: indicator for any positive incentive ($any = 1$)
	    \item Control group: voucher = \$0
	    \end{itemize}
    \item Balanced on pre-treatment characteristics (age, gender, baseline \emph{known} HIV status)
    \item Examined several outcomes (e.g., got test results, learned HIV positive, purchased condoms) using "simple difference in mean outcomes"
        \[
    SDO=\text{mean(got} | \text{any = 1)} - \text{mean(got} | \text{any = 0)}
    \]
