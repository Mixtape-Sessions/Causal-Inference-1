...
kdensity _sim_5, xtitle(Estimated ATT) xline(2622, lwidth(medthick) lpattern(dash) lcolor(blue) extend) xlabel(2622 272) xline(272, lwidth(med) lpattern(solid) lcolor(red) extend) subtitle(Coefficient on Treatment) ytitle("") title("") note("")

graph save "Graph" "./figures/_sim_5.gph", replace

* Figure4: Matching without bias adjustment
kdensity _sim_6, xtitle(Estimated ATT) xline(275, lwidth(medthick) lpattern(dash) lcolor(blue) extend) xlabel(275) xline(272, lwidth(med) lpattern(solid) lcolor(red) extend) subtitle(Age and GPA and no bias adjustment) ytitle("") title("") note("")

graph save "Graph" "./figures/_sim_6.gph", replace

* Figure5: Matching with bias adjustment
kdensity _sim_7, xtitle(Estimated ATT) xline(268, lwidth(medthick) lpattern(dash) lcolor(blue) extend) xlabel(268) xline(272, lwidth(med) lpattern(solid) lcolor(red) extend) subtitle(Age and GPA and bias adjustment) ytitle("") title("") note("")

graph save "Graph" "./figures/_sim_7.gph", replace

graph combine ./figures/_sim_5.gph ./figures/_sim_6.gph ./figures/_sim_7.gph, title(Saturated Regression and Nearest Neighbor Matching) note(The dashed blue line is estimated ATT from 1000 simulations using saturated OLS and nearest neighbor matching.)
