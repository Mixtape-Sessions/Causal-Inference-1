...
* One possibility
twoway (line prob_treat id) (line w1 id, yaxis(2)), ytitle("Share of units treated (blue)") ytitle("OLS weight on ATT (red)", axis(2)) xtitle("Number of units in treatment group (out of 5000 total)") title("Weighted Average Interpretation of Sloczynsi OLS Theorem ") subtitle("Treatment Group Size vs. ATT Weights") note("5000 OLS regressions varying treatment share from 0.001 to 0.9994; dashed vertical line is 50% of sample treated") legend(order(1 "Share of units treated" 2 "OLS weight on ATT") position(6))  xline(2500, lcolor(black) lpattern(dot))
